import { Divisi } from '../../../types';

export const pertanyaanDivisi = [
  {
    divisi: Divisi.Acara,
    q: `Bagaimana pengalamanmu selama mengikuti rangkaian kegiatan MAXIMA 2020 (HoME/STATE/Malam Puncak) kemarin? Jika kamu terpilih nanti, apa yang ingin kamu tingkatkan dari MAXIMA 2020 kemarin?`,
  },
  {
    divisi: Divisi.Bazaar,
    q: `Kontribusi apa yang bisa kamu berikan kepada divisi bazaar maxima jika:
    1. Dilakukan secara online
    2. Dilakukan secara offline
    `,
  },
  {
    divisi: Divisi.Dekorasi,
    q: `Lampirkan portofolio dekorasi yang dikumpulkan dalam format zip (misal konsep atau desain dekorasi yang pernah dibuat. jika memiliki karya showcase 3D, akan menjadi nilai tambah) ke maxima@umn.ac.id. Isi kolom di bawah ini dengan '-'`,
  },
  {
    divisi: Divisi.DokumFoto,
    q: `PORTFOLIO DOKUMENTASI FOTO (WAJIB):

    - 1 Foto Human interest
    - 1 Foto event / stage
    - 2 Foto bebas terbaikmu
    - 2 foto dari after movie maxima (Buka after movie maxima, ambil 2 foto virtual terbaik dan terkreatif) 
    
    Kirim foto-foto tersebut ke maxima@umn.ac.id dengan subject: FOTO_NAMA_ANGKATAN
    Isi kolom di bawah ini dengan '-'
    `,
  },
  {
    divisi: Divisi.DokumVideo,
    q: `PORTFOLIO DOKUMENTASI VIDEO (WAJIB):

    - 1 Video Highlight berdurasi maks 1 Menit (mp4)
    - 1 Video kumpulan Portofoliomu berdurasi 3-5 Menit (mp4)
    - Bedah secara GARIS BESAR Videografi maupun Editing dalam Video Highlight (Instagram @MaximaUMN) dan Aftermovie Maxima 2021 (Youtube MaximaUMN) tanpa minimal kata (Word/PDF)

    Kumpulkan ketiga file tersebut dalam 1 Folder Google Drive masing masing dengan Judul "VIDEO_NAMA_ANGKATAN" dan share link kalian secara publik untuk di kirimkan ke maxima@umn.ac.id
    Isi kolom di bawah ini dengan '-'
    `,
  },
  {
    divisi: Divisi.FreshMoney,
    q: `Seperti yang kita ketahui, sekarang masih dalam masa pandemik sehingga segala kegiatan dilakukan secara online begitu pula dalam mencari dana. Apakah kamu memiliki ide untuk mendapatkan dana nanti?`,
  },
  {
    divisi: Divisi.Medrel,
    q: `Apa yang kamu ketahui tentang sistem alur pengajuan media partner? `,
  },
  {
    divisi: Divisi.Merchandise,
    q: `Wajib melampirkan portofolio terbaik kalian dalam bentuk link Google Drive dengan format nama folder: Nama_NIM_MERCH. 
    Ubah aksesibilitas folder Google Drive menjadi publik dan masukkan link Google Drive di kolom bawah ini!
    `,
  },
  {
    divisi: Divisi.Perlengkapan,
    q: `Mengapa kamu memilih divisi perlengkapan?`,
  },
  {
    divisi: Divisi.Publikasi,
    q: `Bagaimana cara yang kamu lakukan untuk meningkatkan kesadaran dan ketertarikan mahasiswa baru terhadap informasi-informasi mengenai MAXIMA 2021?`,
  },
  {
    divisi: Divisi.Visual,
    q: `Kirimkan portofolio desain grafis terbaikmu dengan basis vector dan desain dengan style kamu (jumlahnya bebas, semakin banyak semakin bagus) ke maxima@umn.ac.id dengan subject: VISUAL-NAMA-ANGKATAN paling lambat Jumat, 12 Februari 2021 pukul 21.00 WIB.  Isi kolom di bawah ini dengan '-' `,
  },
  {
    divisi: Divisi.Seccom,
    q: `Jelaskan yang kamu ketahui tentang Seccom dan tugas divisi tersebut!`,
  },
  {
    divisi: Divisi.Sponsor,
    q: `Terdapat suatu perusahaan besar yang ingin menjalin sponsorship dengan MAXIMA 2021. Perusahaan tersebut hendak memberikan fresh money dengan jumlah yang cukup besar. Sebagai anggota yang bertanggung jawab saat menjalin sponsorship dengan perusahaan terkait, perusahaan tersebut meminta konfirmasi melalui telepon dan konfirmasi langsung tanpa menunggu waktu 'approval' dari koordinator MAXIMA 2021. Apa yang akan Anda lakukan ketika menghadapi situasi tersebut?`,
  },
  {
    divisi: Divisi.Website,
    q: `Kirimkan portofolio/hasil kerja yang terbaikmu yang berkaitan dengan pemrograman/website (jika ada) ke maxima@umn.ac.id dengan subject: WEBSITE/NAMA/ANGKATAN paling lambat 11 Februari 2021 pukul 21.00 WIB.  Isi kolom di bawah ini dengan '-'`,
  },
];
